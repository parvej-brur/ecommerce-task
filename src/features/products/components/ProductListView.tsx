...
export function ProductListView() {
  ...
  if (filters.search) chips.push({ label: `"${filters.search}"`, onRemove: () => setFilters({ search: null, page: 1 }) });
  if (filters.category) chips.push({ label: filters.category, onRemove: () => setFilters({ category: null, page: 1 }) });
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-6">
      ...
      <div className="mb-5 flex flex-wrap items-baseline justify-between gap-2">
        <h1 className="text-2xl font-extrabold tracking-tight text-brand-dark">{listingTitle}</h1>
        {data ? <span className="text-[13px] text-zinc-500">{data.total} products</span> : null}
      </div>
        ...
        <div className="min-w-0 flex-1">
          {chips.length > 0 && (
            <div className="mb-4 flex flex-wrap gap-2">
              {chips.map((chip) => (
                ...
                >
                  {chip.label}
                  <button type="button" onClick={chip.onRemove} aria-label={`Remove ${chip.label} filter`} className="text-sm opacity-60 hover:opacity-100">
                    ×
                  </button>
                </span>
              ))}
          ...
          {error ? (
            ...
            <EmptyState
              ...
              action={
                <Button variant="primary" size="sm" onClick={() => setFilters(RESET_PRODUCT_FILTERS)}>
                  Clear filters
                </Button>
              }
            />
          ) : data ? (
            <div className={cn("transition-opacity", isPreviousData && "opacity-60")}>
              <ProductGrid products={data.products} className="sm:grid-cols-2 lg:grid-cols-3" />
            </div>
          ) : null}
        </div>
      </div>
